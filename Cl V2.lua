...
local Enemies = workspace.Enemies:GetDescendants()
   for i,v in next, Enemies do 
      if v.Parent.Name == "Enemy" and v:IsA("BasePart") and v.Name == "EnemyLocation" and tostring(v.Parent.Model.Value) == CurrentOption and v.Parent.InCombat.Value == false then
         HRP.CFrame = v.CFrame
      end
   end
end)
...
ASection:NewDropdown("Prefer Boss Party", "Automatically teleports you to the mob selected", Mobs, function(CurrentOption)
spawn(function()
...
local Enemies = workspace.Enemies:GetDescendants()
    local CombatFolder = workspace:FindFirstChild("CombatFolder")
        if CombatFolder == nil then
            pcall(function()
            for i,v in next, Enemies do 
                if v.Parent.Name == "Enemy" and v:IsA("BasePart") and tostring(v.Parent.Model.Value) == CurrentOption then
-- and v.Parent.InCombat.Value == false
                    HRP.CFrame = v.CFrame
                end
        end
            end)
...
local Enemies = workspace.Enemies:GetDescendants()
    local CombatFolder = workspace:FindFirstChild("CombatFolder")
        if CombatFolder == nil then
            pcall(function()
            for i,v in next, Enemies do 
                if v.Parent.Name == "Enemy" and v:IsA("BasePart") and tostring(v.Parent.Model.Value) == CurrentOption and v.Parent.InCombat.Value == false then
                    HRP.CFrame = v.CFrame
                end
        end
            end)
...
ASection:NewToggle("Auto OrbTP V2", "Teleports you to orbs automatically (portal relic)", function(State)
    Tpv2 = State
       while Tpv2 and wait() do
       TouchOrb();
...
spawn(function()
while Acts2 and wait(0.1) do
local Level = game:GetService("Players").LocalPlayer:FindFirstChild("leaderstats"):FindFirstChild("Level")
if Level.Value == 400 then
game:GetService("ReplicatedStorage"):WaitForChild("Server"):FireServer(unpack(args))
end
end
end)
...
CSection:NewToggle("Teleport To BM", "Teleports the player to blackmarket", function(State)
    TpB = State
    spawn(function()
            while TpB and wait(.1) do
            local BlackMarket = workspace:FindFirstChild("Stalls"):FindFirstChild("Black Market"):GetDescendants()
                for i,v in pairs(BlackMarket) do
                            if v.Name == "Grani" and v.Parent.Parent.Name == "Grani" then
                                local HRP = game:GetService("Players").LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                                HRP.CFrame = v.CFrame
                                game:GetService("VirtualInputManager"):SendKeyEvent(true, 101, false, game.Players.LocalPlayer.Character.HumanoidRootPart) -- z
                                end
                            end
                            end
    end)
...
CSection:NewToggle("Auto Buy BM", "Teleports the player to blackmarket", function(State)
    AUTOB = State
    task.spawn(function()
            while TpB and wait() do
            local BlackMarket = workspace:FindFirstChild("Stalls"):FindFirstChild("Black Market"):GetDescendants()
                for i,v in pairs(BlackMarket) do
                            if v.Name == "Grani" and v.Parent.Parent.Name == "Grani" then
                                if 1==1 then
...
CSection:NewToggle("Auto Skin", "Teleports the player to blackmarket", function(state)
...
spawn(function()
while wait() do
	local args = {[1]="Invest",[2]="Magic",[3]=30}
...
spawn(function()
while wait() do
	local args = {[1]="Invest",[2]="Mana",[3]=30}
